...
module.exports = {
    ...
    onStart: async ({ event, message, api, args }) => {
        ...
        const endpoint = imgurl
            ? `/tensor/xlv3?prompt=${encodeURIComponent(prompt)}&ratio=${ratio}&weight=${weight}&imgurl=${encodeURIComponent(imgurl)}`
            : `/tensor/xlv3?prompt=${encodeURIComponent(prompt)}&ratio=${ratio}&weight=${weight}`;
        ...
        try {
            ...
            if (response.data.success) {
                ...
                const responseMessage = { body: 'XLV3', attachment: imageStream };
