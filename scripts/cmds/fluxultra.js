  ...
  onStart: async function ({ message, args, api, event }) {
    ...
    try {
      ...
      if (isRestrictedWindow) {
        const firstRes = await axios.get(`https://smfahim.xyz/tensorweb/fluxultra`, { params });
        responseObjects.push(firstRes);
        const secondRes = await axios.get(`https://smfahim.xyz/tensorweb/fluxultra`, { params });
        responseObjects.push(secondRes);
      } else {
        const responsePromises = Array(2).fill(null).map(() =>
          axios.get(`https://smfahim.xyz/tensorweb/fluxultra`, { params })
        );
