module.exports = {
  config: {
    ...
    role: 0,
    longDescription: {
      en: 'Generate an image from text using flux anime.'
    },
    category: 'image',
    guide: {
  ...
  onStart: async function ({ message, api, args, event }) {
    ...
    if (!promptText) {
      return message.reply(`Please enter a text prompt\nExample: \n${global.GoatBot.config.prefix}fluxanime a car or,\n${global.GoatBot.config.prefix}fluxanime a girl --ar 2:3`);
    }
    ...
    try {
      ...
      const team = `fluxanime?prompt=${encodeURIComponent(prompt)}${world}`;
	  const o = "xyz";
      ...
      message.reply({
        body: `Here is your Flux Anime Model 🖼\nTime taken: ${timeTaken} seconds`,
