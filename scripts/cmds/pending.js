...
module.exports = {
  config: {
    ...
    aliases: [],
    version: "1.0",
    author: "S M Fàhîm | Modified mirai to goat bot",
    countDown: 5,
    role: 2,
        ...
        axios({
          ...
          .then(response => {
            response.data.pipe(fs.createWriteStream(filePath))
              .on("finish", () => {
                api.changeNickname(
                  `${!global.GoatBot.config.nickNameBot ? "Made by Alite-Network Team" : global.GoatBot.config.nickNameBot}`,
                  pending[singleIndex - 1].threadID,
                  api.getCurrentUserID()
                );
                api.sendMessage(
                  { body: `${global.GoatBot.config.nickNameBot} Bot successfully connected 🫂🤍 \n\nUse ${global.GoatBot.config.prefix}help to see the Command Lists. \n\n•Join here to learn more about the bot To contract with my Admin: https://www.facebook.com/smfahim.official.bd\n\n•Type ${global.GoatBot.config.prefix}supportgc to join our official group 🙂🤍\n\nOr, join - https://m.me/j/AbYvUbsI8D1d2SJ8/\n\n• Thanks for using this ${global.GoatBot.config.nickNameBot}💝`, attachment: fs.createReadStream(filePath) },
                  pending[singleIndex - 1].threadID
                );
               })
              .on("error", error => {
